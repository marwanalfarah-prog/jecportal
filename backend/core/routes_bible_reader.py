...
def register_bible_reader_routes(app):
    ...
    def serve_bible_books_song():
        ...
        return send_from_directory(
            audio_dir, "AUD000000.mp3",
            as_attachment=as_attachment,
            download_name="أسفار الكتاب المقدس.mp3",
        )

    @app.get("/api/bible-reader/books-song-lyrics")
    def serve_bible_books_song_lyrics():
        err = auth_exports["_require_auth"]()
        if err:
            return err
        lyrics_path = os.path.join(S.db.data_dir, "audios", "AUD000000.json")
        data = S.db.load_json_file(lyrics_path, [])
        return jsonify({"ok": True, "lyrics": data})
